...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return ` ${$$result.head += `<!-- HEAD_svelte-18lvto8_START -->${$$result.title = `<title>Sverdle</title>`, ""}<meta name="description" content="A Wordle clone written in SvelteKit"><!-- HEAD_svelte-18lvto8_END -->`, ""} <h1 class="visually-hidden" data-svelte-h="svelte-16hvqlg">Sverdle</h1> <form method="POST" action="?/enter" class="svelte-1pg2j5l"><div class="${[
    "grid svelte-1pg2j5l",
  ...
  })}</div>`}</div></form> ${won ? `<div style="position: absolute; left: 50%; top: 30%"></div>` : ``}  <img alt="The project logo"${add_attribute("src", logo1, 0)}>`;
});
export {
  Page as default
};
